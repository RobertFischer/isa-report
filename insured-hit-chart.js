...
const calculateData = () => {
  const form = $("#insured-hit-calculator");
  ...
  const person1Hit = _.toInteger($("#alice-hit-year", form).val());
  const person2Hit = _.toInteger($("#bob-hit-year", form).val());
...
$(() => google.charts.setOnLoadCallback(() => {
  const chart = new google.visualization.LineChart(document.getElementById("insured-hit-canvas"));
  const options = {
  ...
  const person1 = $("select#alice-hit-year option[value=10]",form).attr("selected", true);
  const person2 = $("select#bob-hit-year option[value=35]",form).attr("selected", true);
